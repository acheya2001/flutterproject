...
import '../../../widgets/active_session_notification.dart';
import 'sinistres_tab_clean.dart';
import '../../../conducteur/screens/collaborative_choice_screen.dart';
...
class _ConducteurDashboardCompleteState extends State<ConducteurDashboardComplete> {
  ...
  List<Map<String, dynamic>> _brouillons = [];
  ...
  Future<void> _loadSinistres(String userId) async {
    try {
      ...
      // 6. Sessions collaboratives où le conducteur est créateur
      var snapshot6 = await FirebaseFirestore.instance
          .collection('collaborative_sessions')
          .where('conducteurCreateur', isEqualTo: userId)
          .get();
      print('📋 ${snapshot6.docs.length} sessions collaboratives créées trouvées');

      // 7. Sessions collaboratives où le conducteur est participant
      var snapshot7 = await FirebaseFirestore.instance
          .collection('collaborative_sessions')
          .get();

      // Filtrer côté client pour les participants
      var sessionsParticipant = snapshot7.docs.where((doc) {
        final data = doc.data();
      ...
      print('📋 ${sessionsParticipant.length} sessions collaboratives où utilisateur participe');
      ...
      // Ajouter les sessions comme sinistres potentiels avec données enrichies
      for (var doc in snapshot5.docs) {
        if (!seenIds.contains(doc.id)) {
          ...
          final enrichedData = await _enrichirSessionAvecParticipants(doc.id, data);
          ...
          allSinistres.add({
            ...
            'typeAccident': 'Accident collaboratif',
            'type': 'accident_collaboratif',
            'lieu': data['localisation']?['adresse'] ?? 'Non spécifié',
            'lieuAccident': data['localisation']?['adresse'] ?? 'Non spécifié',
            'nombreVehicules': data['nombreVehicules'] ?? 2,
            'progression': _calculerProgressionSession(data),
            ...enrichedData,
          });
        }
      }

      // Ajouter les sessions collaboratives créées par l'utilisateur
      for (var doc in snapshot6.docs) {
        if (!seenIds.contains(doc.id)) {
          seenIds.add(doc.id);
          final data = doc.data() as Map<String, dynamic>;

          // 🆕 Migrer les participants pour ajouter formulaireStatus si manquant
          final participants = (data['participants'] as List<dynamic>? ?? []).map((p) {
            final participant = Map<String, dynamic>.from(p);
            if (!participant.containsKey('formulaireStatus')) {
              // Déterminer l'état basé sur les données existantes
              if (participant['formulaireComplete'] == true || participant['statut'] == 'formulaire_fini') {
                participant['formulaireStatus'] = 'termine';
              } else if (participant['statut'] == 'rejoint') {
                participant['formulaireStatus'] = 'en_cours';
              } else {
                participant['formulaireStatus'] = 'en_attente';
              }
            }
            return participant;
          }).toList();
          ...
          allSinistres.add({
            ...
            'statut': _getCollaborativeSessionStatut(data['statut'] ?? ''),
            'dateCreation': data['dateCreation'],
            'codeSession': data['codeSession'],
            'typeAccident': data['typeAccident'] ?? 'Accident collaboratif',
            'type': 'accident_collaboratif',
            'lieu': data['lieuAccident'] ?? 'Non spécifié',
            'lieuAccident': data['lieuAccident'] ?? 'Non spécifié',
            'nombreVehicules': data['nombreVehicules'] ?? 2,
            'participants': participants, // 🆕 Participants migrés
            'progression': data['progression'] ?? {},
            'estCreateur': true,
            'estCollaboratif': true,
          });
        }
      }

      // Ajouter les sessions collaboratives où l'utilisateur participe
      for (var doc in sessionsParticipant) {
        if (!seenIds.contains(doc.id)) {
          seenIds.add(doc.id);
          final data = doc.data() as Map<String, dynamic>;
  ...
  /// 📊 Convertir le statut de session collaborative en statut sinistre
  String _getCollaborativeSessionStatut(String sessionStatut) {
    switch (sessionStatut) {
      case 'creation':
      case 'attente_participants':
        return 'en_cours';
      case 'en_cours':
        return 'en_cours';
      case 'validation_croquis':
      case 'pret_signature':
        return 'en_cours';
      case 'signe':
      case 'finalise':
  ...
  /// 🔄 Enrichir une session avec les données des participants
  Future<Map<String, dynamic>> _enrichirSessionAvecParticipants(String sessionId, Map<String, dynamic> sessionData) async {
    try {
      print('🔄 Enrichissement session $sessionId avec participants...');

      final participants = sessionData['participants'] as List<dynamic>? ?? [];
      List<Map<String, dynamic>> participantsEnrichis = [];

      for (var participant in participants) {
        final userId = participant['userId'];
        if (userId != null) {
          // Récupérer les données du conducteur
          final conducteurData = await _recupererDonneesConducteur(userId);

          // Récupérer les données du véhicule si disponible
          final vehiculeData = await _recupererDonneesVehicule(participant['vehiculeId']);

          participantsEnrichis.add({
            ...participant,
            'nom': conducteurData['nom'] ?? 'Conducteur',
            'prenom': conducteurData['prenom'] ?? '',
            'telephone': conducteurData['telephone'] ?? '',
            'email': conducteurData['email'] ?? '',
            'vehicule': vehiculeData,
            'donneesCompletes': conducteurData,
          });
        } else {
          participantsEnrichis.add(participant);
        }
      }

      // Récupérer les données du créateur
      final createurId = sessionData['conducteurCreateur'];
      Map<String, dynamic>? createurData;
      if (createurId != null) {
        createurData = await _recupererDonneesConducteur(createurId);
      }

      return {
        ...sessionData,
        'participants': participantsEnrichis,
        'createurData': createurData,
        'nombreParticipants': participantsEnrichis.length,
        'participantsTermines': participantsEnrichis.where((p) => p['statut'] == 'termine').length,
      };

    } catch (e) {
      print('❌ Erreur enrichissement session: $e');
      return sessionData;
    }
  }

  /// 👤 Récupérer les données d'un conducteur
  Future<Map<String, dynamic>> _recupererDonneesConducteur(String userId) async {
    try {
      ...
      // Essayer dans la collection conducteurs
      final docConducteur = await FirebaseFirestore.instance
          .collection('conducteurs')
          .doc(userId)
          .get();

      if (docConducteur.exists) {
        return docConducteur.data() as Map<String, dynamic>;
      }

      return {};
    } catch (e) {
      print('❌ Erreur récupération conducteur $userId: $e');
      return {};
    }
  }

  /// 🚗 Récupérer les données d'un véhicule
  Future<Map<String, dynamic>?> _recupererDonneesVehicule(String? vehiculeId) async {
    ...
    try {
      ...
      print('❌ Erreur récupération véhicule $vehiculeId: $e');
      return null;
    }
  }

  /// 📊 Calculer la progression d'une session
  int _calculerProgressionSession(Map<String, dynamic> sessionData) {
    final participants = sessionData['participants'] as List<dynamic>? ?? [];
    if (participants.isEmpty) return 0;

    final participantsTermines = participants.where((p) => p['statut'] == 'termine').length;
    return ((participantsTermines / participants.length) * 100).round();
  }
  ...
  Widget _buildNewSinistresContent() {
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance
          ...
          .snapshots(),
      builder: (context, snapshot) {
        print('🔍 [SINISTRES] Stream state: ${snapshot.connectionState}');

        if (snapshot.hasError) {
          print('🔍 [SINISTRES] Erreur: ${snapshot.error}');
          return Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
        ...
        print('🔍 [SINISTRES] Sessions trouvées: ${sessions.length}');
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            print('🔍 [SINISTRES] Session ${session.id}: $data');
  ...
  Widget _buildSessionCard(String sessionId, Map<String, dynamic> data) {
    ...
    print('🔍 [SINISTRES] Carte session $sessionId:');
  ...
  void _voirDetailsSession(String sessionId, Map<String, dynamic> data) {
    print('🔍 [SINISTRES] Voir détails session: $sessionId');
  ...
  /// 🚨 En-tête moderne de la page sinistres
  Widget _buildSinistresHeaderModerne() {
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(
        gradient: LinearGradient(
            ...
            children: [
              Expanded(
                child: StreamBuilder<QuerySnapshot>(
                  stream: _getSessionsActivesStream(),
                  builder: (context, snapshot) {
                    int nombreSessionsActives = 0;
                    if (snapshot.hasData) {
                      nombreSessionsActives = snapshot.data!.docs.length;
                    }
                    return _buildStatCardModerne(
                      'Sessions actives',
  ...
  Widget _buildStatCardModerne(String label, String value, IconData icon, Color color) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  /// 📊 Widget pour afficher les statistiques de déclarations d'accidents
  Widget _buildDeclarationsAccidentsStats() {
    return Card(
      elevation: 4,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.assessment, color: Colors.indigo[600], size: 24),
                const SizedBox(width: 12),
                const Text(
                  'Déclarations d\'accidents',
                  style: TextStyle(
            ...
            // Statistiques modernes des déclarations
            _buildModernDeclarationsStats(),
          ],
        ),
      ),
    );
  ...
  /// 📊 Carte de statistique pour les déclarations
  Widget _buildStatCardDeclaration(String label, String value, IconData icon, Color color) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: color.withOpacity(0.1),
  ...
  /// 🚀 Action principale pour déclarer un accident
  Widget _buildActionPrincipale() {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          const Text(
            'Créez une session collaborative ou rejoignez une session existante',
            style: TextStyle(
              color: Colors.white70,
              fontSize: 14,
            ),
          ...
          Row(
            children: [
              ...
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: _rejoindreSessionInscrit,
  ...
  /// 👥 Mes Sessions Collaboratives - Version Moderne
  Widget _buildMesSessionsCollaboratives() {
    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment.topLeft,
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text(
                          '${_sessionsCollaboratives.length} session${_sessionsCollaboratives.length > 1 ? 's' : ''} active${_sessionsCollaboratives.length > 1 ? 's' : ''}',
                          style: TextStyle(
                            color: Colors.white.withOpacity(0.9),
                            fontSize: 14,
                          ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'Créez votre première session pour collaborer\navec d\'autres conducteurs',
            style: TextStyle(
              fontSize: 14,
              color: Colors.grey[600],
              height: 1.5,
          ...
          ElevatedButton.icon(
            onPressed: () {
              // Navigation vers création de session
            },
  ...
  /// 📋 Liste moderne des sessions
  Widget _buildSessionsList() {
    return Column(
      children: [
        // Sessions actives
        ..._sessionsCollaboratives.take(3).map((session) =>
          _buildModernSessionCard(session)
        ).toList(),

        // Bouton voir plus si nécessaire
        if (_sessionsCollaboratives.length > 3) ...[
          const SizedBox(height: 16),
          Container(
            width: double.infinity,
            child: OutlinedButton.icon(
              onPressed: () {
                // Navigation vers toutes les sessions
              },
              icon: const Icon(Icons.visibility_rounded),
              label: Text('Voir toutes les sessions (${_sessionsCollaboratives.length})'),
              style: OutlinedButton.styleFrom(
  ...
  Widget _buildModernSessionCard(Map<String, dynamic> session) {
    final codeSession = session['codePublic'] ?? 'N/A';
    final statut = session['statut'] ?? 'en_attente';
    final dateCreation = session['dateOuverture'] as Timestamp?;
    final conducteurs = session['conducteurs'] as List<dynamic>? ?? [];
    final nombreVehicules = session['nombreVehicules'] ?? conducteurs.length;
    final localisation = session['localisation'] as Map<String, dynamic>?;

    // Calculer les participants
    final participantsRejoints = conducteurs.where((c) => c['aRejoint'] == true).length;
    final progressParticipants = nombreVehicules > 0 ? participantsRejoints / nombreVehicules : 0.0;

    // Couleurs selon le statut
    final statutColors = _getSessionStatutColors(statut);
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: statutColors['color'].withOpacity(0.1),
            blurRadius: 12,
            offset: const Offset(0, 4),
          ),
        ],
        border: Border.all(
          color: statutColors['color'].withOpacity(0.2),
          width: 1,
        ),
      ),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                // Code de session avec style moderne
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                      colors: [statutColors['color'].withOpacity(0.1), statutColors['color'].withOpacity(0.05)],
                    ),
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: statutColors['color'].withOpacity(0.3)),
                  ),
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
              ...
              children: [
                Expanded(
                  child: _buildSessionInfo(
                    ...
                    'Localisation',
                    localisation?['adresse'] ?? 'Non spécifiée',
                    Colors.red[600]!,
                  ),
                ),
                const SizedBox(width: 16),
                Expanded(
                  child: _buildSessionInfo(
                    ...
                    'Créée le',
                    dateCreation != null
                      ? '${dateCreation.toDate().day}/${dateCreation.toDate().month}/${dateCreation.toDate().year}'
                      : 'N/A',
                    Colors.blue[600]!,
                  ),
                ),
            ...
            _buildParticipantsProgress(participantsRejoints, nombreVehicules, progressParticipants),
            ...
            Row(
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () => _voirDetailsSession(session['id'] ?? '', session),
                    icon: const Icon(Icons.visibility_rounded, size: 18),
                    label: const Text('Détails'),
                    style: OutlinedButton.styleFrom(
                      foregroundColor: statutColors['color'],
                      side: BorderSide(color: statutColors['color'].withOpacity(0.5)),
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(10),
                      ),
                    ),
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: () => _rejoindreContinuerSession(session),
                    icon: const Icon(Icons.play_arrow_rounded, size: 18),
                    label: Text(statut == 'en_cours' ? 'Continuer' : 'Rejoindre'),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: statutColors['color'],
                      foregroundColor: Colors.white,
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(10),
                      ),
  ...
  /// 📊 Widget d'information de session
  Widget _buildSessionInfo(IconData icon, String label, String value, Color color) {
    return Container(
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
  ...
  /// 👥 Progression des participants
  Widget _buildParticipantsProgress(int rejoints, int total, double progress) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: LinearGradient(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.people_rounded, size: 18, color: Colors.green[600]),
              const SizedBox(width: 8),
              Text(
                'Participants',
                style: TextStyle(
          ...
          Text(
            progress == 1.0
              ? '✅ Tous les participants ont rejoint'
              : '⏳ En attente de ${total - rejoints} participant${total - rejoints > 1 ? 's' : ''}',
            style: TextStyle(
              fontSize: 12,
  ...
  /// 🎨 Couleurs selon le statut de session
  Map<String, dynamic> _getSessionStatutColors(String statut) {
    switch (statut.toLowerCase()) {
      case 'en_cours':
        return {
  ...
  /// 🚀 Rejoindre ou continuer une session
  void _rejoindreContinuerSession(Map<String, dynamic> session) {
    final statut = session['statut'] ?? 'en_attente';
  ...
  /// 📊 Statistiques modernes des déclarations
  Widget _buildModernDeclarationsStats() {
    // Compter les sinistres individuels (créés directement)
    final sinistresIndividuels = _sinistres.where((s) =>
      s['source'] == 'sinistre' || s['source'] == null
    ).length;

    // Compter les sinistres collaboratifs (créés via sessions)
    final sinistresCollaboratifs = _sinistres.where((s) =>
      s['source'] == 'session'
    ).length;

    final totalSinistres = sinistresIndividuels + sinistresCollaboratifs;
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '$totalSinistres déclaration${totalSinistres > 1 ? 's' : ''} au total',
                          style: TextStyle(
                            color: Colors.white.withOpacity(0.9),
                            fontSize: 12,
                          ),
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: _buildModernStatCard(
                    'Sinistres\nCollaboratifs',
                    sinistresCollaboratifs.toString(),
                    Icons.groups_rounded,
                    Colors.green[600]!,
                    'Sessions multi-conducteurs',
                  ),
                ),
              ],
            ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.drafts, color: Colors.orange[600], size: 24),
                const SizedBox(width: 12),
                const Text(
                  'Mes Brouillons',
                  style: TextStyle(
            ...
            if (_brouillons.isEmpty) ...[
              Container(
                ...
                child: Column(
                  children: [
                    Icon(Icons.note_add, color: Colors.grey[400], size: 48),
                    const SizedBox(height: 12),
                    Text(
                      'Aucun brouillon',
                      style: TextStyle(
                    ...
                    const SizedBox(height: 4),
                    Text(
                      'Vos formulaires en cours apparaîtront ici',
                      style: TextStyle(
              ...
              ..._brouillons.take(3).map((brouillon) =>
                _buildBrouillonCard(brouillon)
              ).toList(),
            ],
          ],
        ),
  ...
  Widget _buildHistoriqueDeclarations() {
    return Card(
      elevation: 4,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_sinistres.isEmpty) ...[
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      'Vos déclarations terminées apparaîtront ici',
                      style: TextStyle(
                        color: Colors.grey[500],
                        fontSize: 12,
                      ),
              ...
              ..._sinistres.take(3).map((sinistre) =>
                _buildSinistreCardModerne(sinistre)
              ).toList(),

              if (_sinistres.length > 3) ...[
                const SizedBox(height: 12),
                TextButton(
                  onPressed: () {
                    // TODO: Naviguer vers l'historique complet
                  },
                  child: Text('Voir tout l\'historique (${_sinistres.length})'),
                ),
              ],
            ],
          ],
  ...
  /// 🔄 Créer nouveau constat
  void _creerNouveauConstat() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const CollaborativeChoiceScreen(
  ...
  /// 📋 Construire la liste des sinistres (méthode qui était orpheline)
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final sinistresEnCours = sinistres.where((data) {
          ...
          print('🔍 [FILTRAGE] Session ${data['id']} - Type: $type - Statut: $statut - StatutSession: $statutSession');

          // Inclure les sessions collaboratives en cours
          if (type == 'collaborative') {
            final isEnCours = ['en_cours', 'en_attente', 'en_attente_participants', 'en_cours_remplissage', 'brouillon', 'creation'].contains(statutSession);
            print('🔍 [FILTRAGE] Session collaborative ${data['id']} - StatutSession: $statutSession - En cours: $isEnCours');
            return isEnCours;
          }

          // Sinistres individuels
          final isEnCours = ['en_attente', 'en_cours', 'en_expertise', 'brouillon'].contains(statut);
          print('🔍 [FILTRAGE] Sinistre individuel ${data['id']} - Statut: $statut - En cours: $isEnCours');
          return isEnCours;
        }).toList();
        ...
        final sinistresTermines = sinistres.where((data) {
          ...
          // Inclure les sessions collaboratives terminées
          if (data['type'] == 'collaborative') {
            return ['termine', 'finalise', 'clos', 'rejete'].contains(statutSession);
          }
        ...
        return Column(
          ...
          children: [
            ...
            if (sinistresEnCours.isNotEmpty) ...[
              ...
              ...sinistresEnCours.take(3).map((data) {
                print('🔍 [DASHBOARD] Sinistre en cours: ${data['id']} - Type: ${data['type']} - Statut: ${data['statut']} - StatutSession: ${data['statutSession']} - Participants: ${data['participants']?.length ?? 0}');

                // Debug détaillé pour les sessions collaboratives
                if (data['type'] == 'collaborative') {
                  print('🔍 [DASHBOARD] SESSION COLLABORATIVE DÉTAILS:');
                  print('  - ID: ${data['id']}');
                  print('  - Code: ${data['codeSession']}');
                  print('  - StatutSession: ${data['statutSession']}');
                  print('  - CreatedBy: ${data['createdBy']}');
                  print('  - Participants: ${data['participants']}');
                  print('  - NombreVehicules: ${data['nombreVehicules']}');
                  print('  - DateCreation: ${data['dateCreation']}');
                  print('  - DerniereMiseAJour: ${data['derniereMiseAJour']}');
                  print('  - Toutes les clés: ${data.keys.toList()}');

                  // Vérifier si les participants ont des données
                  final participants = data['participants'] as List?;
                  if (participants != null) {
                    print('  - Participants détaillés:');
                    for (int i = 0; i < participants.length; i++) {
                      final participant = participants[i];
                      print('    [$i] Type: ${participant.runtimeType}');
                      if (participant is Map) {
                        print('    [$i] Données: $participant');
                      } else {
                        print('    [$i] Valeur: $participant');
                      }
                    }
                  }
                }

                return ModernSinistreCard(
                  sinistre: data,
          ...
          FutureBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              ...
              return Padding(
                padding: const EdgeInsets.all(16),
                child: Column(
                  children: brouillons.map<Widget>((brouillon) => _buildBrouillonCard(brouillon)).toList(),
                ),
              );
            },
          ),
  ...
  /// 📋 Construire une carte de sinistre moderne
  Widget _buildSinistreCardModerne(Map<String, dynamic> sinistre) {
    return Card(
      elevation: 2,
      margin: const EdgeInsets.only(bottom: 12),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
          ...
          children: [
            Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.assignment, color: Colors.green[600], size: 20),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        sinistre['numeroSinistre'] ?? 'Sinistre',
                        style: const TextStyle(
                          fontWeight: FontWeight.bold,
                          fontSize: 16,
                        ),
                      ...
                      Text(
                        sinistre['dateAccident'] ?? 'Date inconnue',
                        style: TextStyle(
                          color: Colors.grey[600],
                          fontSize: 12,
                        ),
                ...
                Container(
                  ...
                  child: Text(
                    _getStatusText(sinistre['statut']),
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 10,
                      fontWeight: FontWeight.bold,
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    sinistre['lieuAccident'] ?? 'Lieu non spécifié',
  ...
  /// 📡 Stream des sessions actives de l'utilisateur
  Stream<QuerySnapshot> _getSessionsActivesStream() {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) {
  ...
  void _ouvrirSessionCollaborative(CollaborativeSession session) {
    print('Ouverture session: ${session.id}');
    // TODO: Implémenter la navigation vers SessionDashboardScreen
  }

  /// 🚪 Ouvrir le dashboard de session
  void _ouvrirSessionDashboard(Map<String, dynamic> sessionData) {
    print('Ouverture session dashboard: ${sessionData['id']}');
    // TODO: Implémenter la navigation vers CollaborativeSessionDashboard
  }
}
